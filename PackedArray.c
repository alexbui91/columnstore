 ...
 *      Author: alex
 *      reference url: https://github.com/gpakosz/PackedArray/blob/master/PackedArray.h
 */
...
PackedArray* PackedArray_create(uint32_t bitsPerItem, uint32_t count) {
	...
	bufferSize = sizeof(uint32_t)
			* (((uint64_t) bitsPerItem * (uint64_t) count + 31) / 32);
	a = (PackedArray*) PACKEDARRAY_MALLOC(sizeof(PackedArray) + bufferSize);

	if (a != NULL) {
		a->buffer[((uint64_t) bitsPerItem * (uint64_t) count + 31) / 32 - 1] =
				0;
...
void PackedArray_pack(PackedArray* a, const uint32_t offset, const uint32_t* in,
		uint32_t count) {
...
void PackedArray_unpack(const PackedArray* a, const uint32_t offset,
...
void PackedArray_set(PackedArray* a, const uint32_t offset, const uint32_t in) {
	...
	uint32_t tmp = (uint32_t)(~mask & in);
//  PACKEDARRAY_ASSERT(0 == tmp);
	if (tmp == 0) {
...
uint32_t PackedArray_bufferSize(const PackedArray* a) {
	PACKEDARRAY_ASSERT(a != NULL);
	return (uint32_t) (((uint64_t) a->bitsPerItem * (uint64_t) a->count + 31)
			/ 32);
}
